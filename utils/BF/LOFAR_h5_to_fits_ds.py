...
def parse_args():
    ...
    parser.add_argument('--t_c_ratio', help='compression ratio of the time', type=int, default=24)
    parser.add_argument('--f_c_ratio', help='compression ratio of the frequency', type=int, default=8)

    parser.add_argument('--minutes_per_chunk', help='time chunks in minutes', type=int, default=15)
    parser.add_argument('--num_chunks', help='number of output chunks, -1 means to the end', type=int, default=-1)
    ...
    parser.add_argument('--t_idx_cut', help='length of t-index for convolution, \
        8192 means about 195MB for 6400 channels', type=int, default=128)
...
def compress_h5(fname_DS,
    ...
    print('data shape [t,f]:', t_lines, f_lines)
    ...
    t_all = np.linspace(mdates.date2num(t_start_bf), mdates.date2num(t_end_bf), t_lines)
    ...
    if num_chunks == -1:
        chunk_num = int(np.max([np.floor((t_end_bf - t_start_chunk) / minutes_per_chunk), 1]))
    else:
        chunk_num = num_chunks

    f_fits = avg_1d(freq, f_c_ratio)
    ...
    for idx_cur in range(chunk_num):
        ...
        if averaging:
            ...
            for idx_segment in np.arange(num_segments):
                stokes_tmp = f[dataset_uri][(idx_start + idx_segment * segment_len):np.min([
                ...
                if t_tmp.shape[0] >= t_c_ratio - 1:
                    if flagging:
                        ...
                        conv_down_after_flag = F.conv2d(
                            torch.tensor(stokes_tmp[None, None, :, :] * (~output[None, None, :, :])),
                        ...
                        conv_down_weight_after_flag = F.conv2d(torch.tensor(1 - output[None, None, :, :]) * 1.0,
                                                               ...
                                                               stride=(time_window, freq_window),
        ...
        if date_directory:
            real_out_dir = os.path.join(real_out_dir, t_start_fits.strftime(r"%Y/%m/%d"))
            
        os.makedirs(real_out_dir, exist_ok=True)
        if simple_fname:
            fname = t_start_fits.strftime(r"%Y%m%d_%H")+"_S"+stokes_key.strip()[-1]  # + '.fits'
        ...
        if ('0' not in stokes_key.lower()): # QUV
            ...
            # scale vmax and vmin
            freq_safe0, freq_safe1 = int(0.27 * f_fits.shape[0]), int(0.99 * f_fits.shape[0])
        ...
        # np.tile(np.nanmean(data_fits, 0), (data_fits.shape[0], 1))
        im = ax.imshow(data_fits_new.T, aspect='auto', origin='lower',vmax=vmax,vmin=vmin,
