...
#from sunpy.coordinates import get_sun_P ## deprecated from sunpy 1.0
...
def get_cur_solar_centroid(header):
    ...
    [RA,DEC] = sun_position(t=sunpy.time.parse_time(header['DATE-OBS']))
...
def sun_coord_trasform(data,header,act_r=True,act_s=False):
    ...
    if act_s==False:
        x_shift_pix = 0
        y_shift_pix = 0
    rotate_angel = sun_coord.P(sunpy.time.parse_time(header['DATE-OBS'])).degree
    if act_r==False:
        rotate_angel = 0
...
def plot_image(fname,vmax_set=np.nan,log_scale=False):
    ...
    solar_PA = sun_coord.P(sunpy.time.parse_time(fits.open(fname)[0].header['DATE-OBS'])).degree
